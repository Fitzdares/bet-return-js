...
import { toDecimal } from './conversions';
...
function single({
  ...
  const placeReturn = ew ? bet(newPlaceStake, ((decimalOdds - 1) * terms) + 1) : 0;
...
function effectiveOdds(selections) {
  return selections.reduce((acc, selection) => acc * toDecimal(selection.odds), 1);
}
